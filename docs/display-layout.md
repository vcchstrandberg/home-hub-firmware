...
240×320 stock ST7789 panel driven by Arduino_GFX + LVGL 8.4 — see `src/lvgl_ui_s3.cpp`. This board has **no touch panel**. Until 2026-08-16 that meant no paging at all (indoor/outdoor/rain/forecast all shown at once in a single 2×2 grid); an external KY-040 rotary encoder wired to the board's header (see [wiring.md](wiring.md)) now drives a **four-page carousel**, mirroring the C6 Touch LCD's swipe-driven pages but with one extra page since rain rolls into "Outside" instead of getting a fifth always-on card:

1. **Inside** — indoor name, a 48pt hero temperature, and a 20pt humidity line, centered on an otherwise empty page.
2. **Outside** — same hero temperature treatment for outdoor temp/pressure (plus the high-pressure sun icon), with a rain block (1h/24h at 32pt, droplet indicator) grouped below it by spacing and accent color.
3. **Weather** — tomorrow's forecast: title, an 84×84 drawn weather icon, 48pt hi/lo temps, precipitation.
4. **About** — app name, version, build info, and a QR code linking to the repo (new on this board).

None of the content pages use a bordered "card" — a first pass reused the old quarter-grid card widgets just stretched to fill the screen, which read as "a blown-up version of the same components" rather than a real full-screen page; the pages now use the same borderless, centered-flex composition the Weather/About pages always had, with generously larger type (a 36pt→48pt / 24pt→32pt / 14pt→20pt scale-up, `LV_FONT_MONTSERRAT_48`/`_32` newly enabled in `lv_conf.h`) so the temperature reads as the page's actual focal point. Encoder rotation advances/retreats one page per detent; the encoder's built-in push button cycles locale. Same 30 s auto-return-to-page-0 timeout as the C6 board if there's no input. Page-indicator dots (4, muted/white) sit in a reserved bottom band on every page, same chrome as the C6 board's carousel. All four pages are built by `buildPages()` from the panel's actual width/height, so the same code produces both landscape (320×240) and portrait (240×320) — no separate per-orientation layout functions, matching this board's existing "compute from `s_gfx->width()/height()`" approach.
...
**Modal overlays:** same boot splash / connecting / locale / error overlays as the C6 board, full-screen, drawn on top of whichever page is active.
