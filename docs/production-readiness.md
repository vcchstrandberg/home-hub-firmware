...
**Status: verified end-to-end 2026-08-08.** First test run (before the pinning fix) built 4/6 targets successfully and correctly skipped `publish` because of the other two's failure (`needs: build` only runs `publish` if every matrix leg succeeds — conservative, since the `publish` job's copy loop has no per-target error handling and would otherwise crash mid-loop on a missing artifact). After the pinning fix and the `esp32cam`/`esp32dev` removal, the `v2.22` tag push triggered a full run automatically (the workflow's own `on: push: tags: 'v*'`) — all four targets built and published cleanly, `netatmo-home-hub/server/firmware/<env>/version.txt` reading exactly `2.22` for each. This also published `esp32c6_zero`'s binary for the first time ever (previously only hand-copied builds for the other three had made it into the repo) — it's available to pull, just not yet meaningful until that board is next connected. One thing to avoid next release: a manual `workflow_dispatch` run right after the tag push is redundant (the tag push already triggers the workflow) — it happened here and produced a harmless but pointless second publish commit.
